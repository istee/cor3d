...
void EditSkeleton::update()
{
    ...
    if (selected >= 0)
    {
        ...
        model_file_label->setText(QString::fromStdString(Cor3d::getInstance().get_skeleton_model_file(selected)));
        model_x_doubleSpinBox->setValue(Cor3d::getInstance().get_skeleton_model_x(selected));
        model_y_doubleSpinBox->setValue(Cor3d::getInstance().get_skeleton_model_y(selected));
        model_z_doubleSpinBox->setValue(Cor3d::getInstance().get_skeleton_model_z(selected));
    }
    else
    {
        disable_edits();
