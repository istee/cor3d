...
namespace cagd
{
    ...
    public:
        ...
        bool ResizeRows(GLuint row_count);
        bool ResizeColumns(GLuint row_count);

        // tries to determine the LU decomposition of this square matrix
        GLboolean PerformLUDecomposition();

        // Solves linear systems of type A * b = x, where A is a regular square matrix,
...
//    RealSquareMatrix::RealSquareMatrix(unsigned int size): Matrix<GLdouble>(size, size), _lu_decomposition_is_done(false)
//    {
//    }

    inline bool RealSquareMatrix::ResizeRows(unsigned int row_count)
    {
        return (Matrix<double>::ResizeRows(row_count) && Matrix<GLdouble>::ResizeColumns(row_count));
    }

    inline bool RealSquareMatrix::ResizeColumns(unsigned int column_count)
    {
        return (Matrix<double>::ResizeRows(column_count) && Matrix<GLdouble>::ResizeColumns(column_count));
    }
